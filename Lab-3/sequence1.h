...
namespace coen79_lab3 {
    ...
    public:
        ...
        // Constructor
        sequence() {
            // Set the total number of elements to 0 initially
            total = 0;
            // Set the iterator to 0 initially
            iterator = 0;
        };

        // Modification Member Functions
        void start();                               // The iterator is reset to position 0
        void end();                                 // The iterator points to the last item in the sequence
        void last();                                // The iterator is reset to CAPACITY - 1, regardless of the numebr of items in the sequence
        void advance();                             // Move to the next item in the list
        void retreat();                             // Move to the previous item in the list
        void insert(const value_type& entry);       // Insert a new item into the array sequence
        void attach(const value_type& entry);       // A new copy of entry has been inserted in the sequence after the current item
        void remove_current();                      // Remove the current item from the sequence
        ...
        bool is_item( ) const;                      // Indicates whether there's a valid "current" item that can be retrieved by activating the current member function (listed below)
        value_type current() const;                 // Returns the item in the sequence at the current position of the iterator
        value_type operator[](int index) const;     // Returns the item stored at "index"
        double mean() const;                        // Returns the mean of the values stored in the sequence
        double standardDeviation() const;           // Returns the stadard deviation of the values stored in the sequence

    private:
        // Member Variables
        size_type iterator;         // Iterator to keep track of insertion, etc
        size_type total;            // Total elements in the array
        value_type array[CAPACITY]; // Array holding elements
    };
